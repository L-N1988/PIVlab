...
elseif valid_session_file == 1
    ...
    if isfield(vars,'stereo_session')
        gui.put('stereo_session',vars.stereo_session);
    elseif vars.stereomode == 1
        stereo.init_gui_state();
    end
    if isfield(vars,'stereo_calibration')
        gui.put('stereo_calibration',vars.stereo_calibration);
    elseif vars.stereomode == 1
        stereo.ensure_coupled_calibration();
    end
    import.update_stereo_import_controls();
    stereo.update_menu_state();
    if vars.stereomode == 1
